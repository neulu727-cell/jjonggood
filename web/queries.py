...
def get_siblings(db: DatabaseManager, phone: str, exclude_id: int = None) -> list:
    ...
    if exclude_id:
        rows = db.fetch_all(
            "SELECT id, pet_name, breed, weight, age, memo, keyring FROM customers WHERE phone = ? AND id != ? ORDER BY id",
            (phone, exclude_id))
    else:
        rows = db.fetch_all(
            "SELECT id, pet_name, breed, weight, age, memo, keyring FROM customers WHERE phone = ? ORDER BY id",
            (phone,))
...
def get_reservation_names_by_month(db: DatabaseManager, year: int, month: int) -> dict:
    rows = db.fetch_all(
        """SELECT TO_CHAR(r.date, 'YYYY-MM-DD') as date_str,
                  ...
                  c.pet_name, c.breed, c.phone, r.service_type, r.status, r.duration
           FROM reservations r
    ...
    for row in rows:
        ...
        result[d].append({
            ...
            "is_boss": row["phone"] == "BOSS",
            "is_allday": row["phone"] == "BOSS" and (row.get("duration") or 0) >= 480,
