...
def get_siblings(db: DatabaseManager, phone: str, exclude_id: int = None) -> list:
    ...
    if exclude_id:
        rows = db.fetch_all(
            "SELECT id, pet_name, breed, weight, age, memo FROM customers WHERE phone = ? AND id != ? ORDER BY id",
            (phone, exclude_id))
    else:
        rows = db.fetch_all(
            "SELECT id, pet_name, breed, weight, age, memo FROM customers WHERE phone = ? ORDER BY id",
            (phone,))
